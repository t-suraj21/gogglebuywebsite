...
export default function ChildWear() {
  ...
  // Filter products for child wear - includes kids glasses or child category
  const childProducts = gogglesProducts.filter(p => 
    p.category?.toLowerCase().includes("kids") || 
    p.category?.toLowerCase().includes("child")
  ).length > 0 ? 
    gogglesProducts.filter(p => 
      p.category?.toLowerCase().includes("kids") || 
      p.category?.toLowerCase().includes("child")
    ) 
    : gogglesProducts.slice(0, 10); // Fallback to first 10 if no kids products

  const brands = [...new Set(childProducts.map(p => p.brand))].filter(Boolean);

  const filtered = childProducts.filter((product) => {
  ...
  return (
    <div className="min-h-screen bg-white w-full">
      ...
      <section className="bg-blue-50 border-b">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex flex-wrap items-center justify-center gap-6 py-3 text-sm font-semibold">
            ...
            <span className="text-gray-400">/</span>
            <Link to="/child-wear" className="text-blue-600 font-bold">Child Wear</Link>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white py-8 border-b">
        <div className="max-w-7xl mx-auto px-4">
          <h1 className="text-4xl font-bold text-gray-900">Child Wear Eyeglasses</h1>
          <p className="text-gray-600 mt-2">Premium eyewear collection for children</p>
        </div>
      </section>
