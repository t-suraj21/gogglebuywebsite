...
export default function FemaleWear() {
  ...
  // Filter products for female wear - you can customize this filter based on your product data
  const femaleProducts = gogglesProducts.filter(p => 
    !p.category?.toLowerCase().includes("kids") && 
    !p.category?.toLowerCase().includes("child")
  );

  const brands = [...new Set(femaleProducts.map(p => p.brand))].filter(Boolean);

  const filtered = femaleProducts.filter((product) => {
  ...
  return (
    <div className="min-h-screen bg-white w-full">
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8 py-3">
          ...
          {isMenuOpen && (
            <div className="md:hidden mt-4 pb-4 space-y-3 border-t pt-4">
              ...
              <Link to="/female-wear" className="block px-3 py-2 text-gray-700 hover:bg-blue-50 rounded font-semibold text-blue-600">Female Wear</Link>
      ...
      <section className="bg-blue-50 border-b">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex flex-wrap items-center justify-center gap-6 py-3 text-sm font-semibold">
            ...
            <Link to="/male-wear" className="text-gray-700 hover:text-blue-600 transition">Men Wear</Link>
            <span className="text-gray-400">/</span>
            <Link to="/female-wear" className="text-blue-600 font-bold">Female Wear</Link>
            <span className="text-gray-400">/</span>
      ...
      <section className="bg-white py-8 border-b">
        <div className="max-w-7xl mx-auto px-4">
          <h1 className="text-4xl font-bold text-gray-900">Female Wear Eyeglasses</h1>
          <p className="text-gray-600 mt-2">Premium eyewear collection for women</p>
        </div>
      </section>
